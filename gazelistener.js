...
var cx,
  cy,
  vx,
  vy = 0; // Cursor varaibles (update to class if necessary)
...
function isOverListener() {
  ...
  if (prediction) {
    ...
    elements.forEach(function (id) {
      ...
      let height =
        parseInt(heightStr.substring(0, heightStr.length - 2)) + buffer;
      ...
      let inside =
        ((left < left_side && left_side < left + width) ||
          (left < right_side && right_side < left + width)) &&
        ((top < top_side && top_side < top + height) ||
          (top < bottom_side && bottom_side < top + height));
...
function updateCursor(point, circleSize) {
  ...
  if (!CursorDrawn) {
    // Adds the cursor Element to the webpage
    drawCursor();
    CursorDrawn = true;
  }
