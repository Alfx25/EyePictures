...
function gotoPage(url, dir) {
  ...
  var oldColor = window
    .getComputedStyle(document.getElementById("border-" + dir), null)
  ...
  client.onload = function () {
    ...
    var content = page.substring(
      page.indexOf('<div id="content">') + 18,
      page.indexOf("</div> <!-- end of content -->")
    );
    var title = page.substring(
...
function quadroAvanti() {
  ...
  if (file.includes("bacio")) {
    document
      .getElementById("immagine")
      .setAttribute("src", "/film/gioconda.jpeg");
  }
...
function openImage(url, quadri) {
  ...
  document.getElementById(quadri).style =
    "top: -50px; left: -50px; overflow: visible; transition: all 1s ease; z-index: 2000;";
  document.getElementById(quadri + "-img").style =
    "filter: brightness(0%); width: 100vw; height: 100vh; top: 0; left: 0; transition: width 1s ease, height 0.5s ease";
  ...
  client.onload = function () {
    ...
    setTimeout(function () {
      ...
      document
        .getElementById("immagine")
        .setAttribute("src", "/quadri/" + quadri + ".jpeg");
    }, 1000);
  };
  client.send();
}
